...
class cPluginHandler:
    ...
    def __updateSettings(self, pluginDB):
        ...
        try:
            ...
            logger.debug(f'-> [pluginHandler]: settings.xml erfolgreich aktualisiert. Plugins entfernt: {self._removed_plugins}')
    ...
    def checkDomain(self):
        ...
        for fileName in fileNames:
            try:
                ...
                wrongDomain = 'site-maps.cc', 'drei.at', 'notice.cuii.info', 'streamingunity.biz'
    ...
    def _checkdomain(self, provider, base_link):
        try:
            ...
            # Status 301 - richtet Ihr auf Eurem Server ein, wenn sich die URL geändert hat, Eure Domain umgezogen ist oder sich ein Inhalt anderweitig verschoben hat.
                ...
                if redirected_domain in wrongDomain:  # Falsche Umleitung ausschliessen
                    cConfig().setSetting('plugin_' + provider + '.domain', '')
                    cConfig().setSetting('global_search_' + provider, 'false')
                    logger.debug('-> [checkDomain]:Redirect to wrong domain: ' + redirected_domain + ' for ' + provider)
                else:
